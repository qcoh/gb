	...
	cycles{0}
{
	instructions = {{
		{ 0x00, [](){}, "NOP", 4, 0 },
		{ 0x01, std::bind(&CPU::LD<WORD, WORD>, 	this, std::ref(bc), std::cref(nn)), 	"LD BC, nn",	12, 2 },
		{ 0x02, std::bind(&CPU::LD<MemRef, BYTE>,	this, MemRef{bc, mmu}, std::cref(a)),	"LD (BC), A",	8, 0 },
		{ 0x03, std::bind(&CPU::INC,			this, std::ref(bc)),			"INC BC",	8, 0 },
		{ 0x04, std::bind(&CPU::INCb,			this, std::ref(b)),			"INC B",	4, 0 },
		{ 0x05, std::bind(&CPU::DECb,			this, std::ref(b)),			"DEC B",	4, 0 },
		{ 0x06, std::bind(&CPU::LD<BYTE, BYTE>, 	this, std::ref(b), std::cref(n)), 	"LD B, n",	8, 1 },
		{ 0x07, std::bind(&CPU::RLCA,			this),					"RLCA",		4, 0 },
		{ 0x08, std::bind(&CPU::LD<MemRef, WORD>,	this, MemRef{nn, mmu}, std::cref(sp)),	"LD (nn), SP",	20, 2 },
		{ 0x09, std::bind(&CPU::ADD16,			this, std::ref(hl), std::cref(bc)),	"ADD HL, BC",	8, 0 },
		{ 0x0a, std::bind(&CPU::LD<BYTE, MemRef>,	this, std::ref(a), MemRef{bc, mmu}),	"LD A, (BC)",	8, 0 },
		{ 0x0b, std::bind(&CPU::DEC,			this, std::ref(bc)), 			"DEC BC", 	8, 0 },
		{ 0x0c, std::bind(&CPU::INCb,			this, std::ref(c)),			"INC C",	4, 0 },
		{ 0x0d, std::bind(&CPU::DECb,			this, std::ref(c)),			"DEC C",	4, 0 },
		{ 0x0e, std::bind(&CPU::LD<BYTE, BYTE>, 	this, std::ref(c), std::cref(n)), 	"LD C, n", 	8, 1 },
		{ 0x0f, std::bind(&CPU::RRCA,			this),					"RRCA",		4, 0 },

		{},
		{ 0x11, std::bind(&CPU::LD<WORD, WORD>, 	this, std::ref(de), std::cref(nn)), 	"LD DE, nn", 	12, 2 },
		{ 0x12, std::bind(&CPU::LD<MemRef, BYTE>,	this, MemRef{de, mmu}, std::cref(a)),	"LD (DE), A",	8, 0 },
		{ 0x13, std::bind(&CPU::INC, 			this, std::ref(de)), 			"INC DE", 	8, 0 },
		{ 0x14, std::bind(&CPU::INCb, 			this, std::ref(d)),			"INC D",	4, 0 },
		{ 0x15, std::bind(&CPU::DECb, 			this, std::ref(d)),			"DEC D",	4, 0 },
		...
		{ 0x19, std::bind(&CPU::ADD16,			this, std::ref(hl), std::cref(de)),	"ADD HL, DE",	8, 0 },
		{ 0x1a, std::bind(&CPU::LD<BYTE, MemRef>,	this, std::ref(a), MemRef{de, mmu}),	"LD A, (DE)",	8, 0 },
		{ 0x1b, std::bind(&CPU::DEC, 			this, std::ref(de)), 			"DEC DE", 	8, 0 },
		{ 0x1c, std::bind(&CPU::INCb,			this, std::ref(e)),			"INC E",	4, 0 },
		{ 0x1d, std::bind(&CPU::DECb,			this, std::ref(e)),			"DEC E",	4, 0 },
		{ 0x1e, std::bind(&CPU::LD<BYTE, BYTE>, 	this, std::ref(e), std::cref(n)), 	"LD E, n", 	8, 1 },
		{ 0x1f, std::bind(&CPU::RRA,			this),					"RRA",		4, 0 },

		{ 0x20, std::bind(&CPU::JRn,			this, zeroFlag, std::cref(n)),		"JR NZ, n",	0, 0 },
		{ 0x21, std::bind(&CPU::LD<WORD, WORD>, 	this, std::ref(hl), std::cref(nn)), 	"LD HL, nn", 	12, 2 },
		{ 0x22, std::bind(&CPU::LDI<MemRef, BYTE>,	this, MemRef{hl, mmu}, std::cref(a)),	"LDI (HL+), A", 8, 0 },
		{ 0x23, std::bind(&CPU::INC, 			this, std::ref(hl)), 			"INC HL", 	8, 0 },
		{ 0x24, std::bind(&CPU::INCb, 			this, std::ref(h)),			"INC H",	4, 0 },
		{ 0x25, std::bind(&CPU::DECb, 			this, std::ref(h)),			"DEC H",	4, 0 },
		{ 0x26, std::bind(&CPU::LD<BYTE, BYTE>, 	this, std::ref(h), std::cref(n)), 	"LD H, n", 	8, 1 },
		{ 0x27, std::bind(&CPU::DAA,			this),					"DAA",		4, 0 },
		{ 0x28, std::bind(&CPU::JR,			this, zeroFlag, std::cref(n)),		"JR Z, n",	0, 0 },
		{ 0x29, std::bind(&CPU::ADD16,			this, std::ref(hl), std::cref(hl)),	"ADD HL, HL",	8, 0 },
		{ 0x2a, std::bind(&CPU::LDI<BYTE, MemRef>,	this, std::ref(a), MemRef{hl, mmu}),	"LDI A, (HL+)", 8, 0 },
		{ 0x2b, std::bind(&CPU::DEC, 			this, std::ref(hl)), 			"DEC HL", 	8, 0 },
		{ 0x2c, std::bind(&CPU::INCb,			this, std::ref(l)),			"INC L",	4, 0 },
		{ 0x2d, std::bind(&CPU::DECb,			this, std::ref(l)),			"DEC L",	4, 0 },
		{ 0x2e, std::bind(&CPU::LD<BYTE, BYTE>, 	this, std::ref(l), std::cref(n)), 	"LD L, n", 	8, 1 },
		{ 0x2f, std::bind(&CPU::CPL,			this),					"CPL",		4, 0 },

		{ 0x30, std::bind(&CPU::JRn,			this, carryFlag, std::cref(n)),		"JR NC, n",	0, 0 },
		...
		{ 0x3a, std::bind(&CPU::LDD<BYTE, MemRef>,	this, std::ref(a), MemRef{hl, mmu}),	"LDD A, (HL-)", 8, 0 },
		{ 0x3b, std::bind(&CPU::DEC,			this, std::ref(sp)), 			"DEC SP", 	8, 0 },
		{ 0x3c, std::bind(&CPU::INCb,			this, std::ref(a)),			"INC A",	4, 0 },
		{ 0x3d, std::bind(&CPU::DECb,			this, std::ref(a)),			"DEC A",	4, 0 },
		...
		{ 0x46, std::bind(&CPU::LD<BYTE, BYTE>,	this, std::ref(b), MemRef{hl, mmu}),	"LD B, (HL)",	8, 0 },
		...
		{ 0x4e, std::bind(&CPU::LD<BYTE, BYTE>,	this, std::ref(c), MemRef{hl, mmu}),	"LD C, (HL)",	8, 0 },
		...
		{ 0x56, std::bind(&CPU::LD<BYTE, BYTE>,	this, std::ref(d), MemRef{hl, mmu}),	"LD D, (HL)",	8, 0 },
		...
		{ 0x5e, std::bind(&CPU::LD<BYTE, BYTE>,	this, std::ref(e), MemRef{hl, mmu}),	"LD E, (HL)",	8, 0 },
		...
		{ 0x66, std::bind(&CPU::LD<BYTE, BYTE>,	this, std::ref(h), MemRef{hl, mmu}),	"LD H, (HL)",	8, 0 },
		...
		{ 0x6e, std::bind(&CPU::LD<BYTE, BYTE>,	this, std::ref(l), MemRef{hl, mmu}),	"LD L, (HL)",	8, 0 },
		{ 0x6f, std::bind(&CPU::LD<BYTE, BYTE>,	this, std::ref(l), std::cref(a)),	"LD L, A",	4, 0 },

		{ 0x70, std::bind(&CPU::LD<MemRef, BYTE>, this, MemRef{hl, mmu}, std::cref(b)),	"LD (HL), B",	8, 0 },
		{ 0x71, std::bind(&CPU::LD<MemRef, BYTE>, this, MemRef{hl, mmu}, std::cref(c)),	"LD (HL), C",	8, 0 },
		{ 0x72, std::bind(&CPU::LD<MemRef, BYTE>, this, MemRef{hl, mmu}, std::cref(d)),	"LD (HL), D",	8, 0 },
		{ 0x73, std::bind(&CPU::LD<MemRef, BYTE>, this, MemRef{hl, mmu}, std::cref(e)),	"LD (HL), E",	8, 0 },
		{ 0x74, std::bind(&CPU::LD<MemRef, BYTE>, this, MemRef{hl, mmu}, std::cref(h)),	"LD (HL), H",	8, 0 },
		{ 0x75, std::bind(&CPU::LD<MemRef, BYTE>, this, MemRef{hl, mmu}, std::cref(l)),	"LD (HL), L",	8, 0 },
		{}, // 0x76
		{ 0x77, std::bind(&CPU::LD<MemRef, BYTE>, this, MemRef{hl, mmu}, std::cref(a)),	"LD (HL), A",	8, 0 },
		...
		{ 0x7e, std::bind(&CPU::LD<BYTE, BYTE>,	this, std::ref(a), MemRef{hl, mmu}),	"LD A, (HL)",	8, 0 },
		{ 0x7f, std::bind(&CPU::LD<BYTE, BYTE>,	this, std::ref(a), std::cref(a)),	"LD A, A",	4, 0 },

		{ 0x80, std::bind(&CPU::ADD,		this, std::cref(b)),			"ADD A, B",	4, 0 },
		{ 0x81, std::bind(&CPU::ADD,		this, std::cref(c)),			"ADD A, C",	4, 0 },
		{ 0x82, std::bind(&CPU::ADD,		this, std::cref(d)),			"ADD A, D",	4, 0 },
		{ 0x83, std::bind(&CPU::ADD,		this, std::cref(e)),			"ADD A, E",	4, 0 },
		{ 0x84, std::bind(&CPU::ADD,		this, std::cref(h)),			"ADD A, H",	4, 0 },
		{ 0x85, std::bind(&CPU::ADD,		this, std::cref(l)),			"ADD A, L",	4, 0 },
		{ 0x86, std::bind(&CPU::ADD,		this, MemRef{hl, mmu}),			"ADD A, (HL)",	8, 0 },
		{ 0x87, std::bind(&CPU::ADD,		this, std::cref(a)),			"ADD A, A",	4, 0 },
		...
		{ 0x8e, std::bind(&CPU::ADC,		this, MemRef{hl, mmu}),			"ADC A, (HL)",	8, 0 },
		...
		{ 0x96, std::bind(&CPU::SUB,		this, MemRef{hl, mmu}),			"SUB A, (HL)",	8, 0 },
		...
		{ 0x9e, std::bind(&CPU::SBC,		this, MemRef{hl, mmu}),			"SBC A, (HL)",	8, 0 },
		{ 0x9f, std::bind(&CPU::SBC,		this, std::cref(a)),			"SBC A, A",	4, 0 },

		{ 0xa0, std::bind(&CPU::AND,		this, std::cref(b)),			"ADD A, B",	4, 0 },
		{ 0xa1, std::bind(&CPU::AND,		this, std::cref(c)),			"ADD A, C",	4, 0 },
		{ 0xa2, std::bind(&CPU::AND,		this, std::cref(d)),			"ADD A, D",	4, 0 },
		{ 0xa3, std::bind(&CPU::AND,		this, std::cref(e)),			"ADD A, E",	4, 0 },
		{ 0xa4, std::bind(&CPU::AND,		this, std::cref(h)),			"ADD A, H",	4, 0 },
		{ 0xa5, std::bind(&CPU::AND,		this, std::cref(l)),			"ADD A, L",	4, 0 },
		{ 0xa6, std::bind(&CPU::AND,		this, MemRef{hl, mmu}),			"ADD A, (HL)",	8, 0 },
		{ 0xa7, std::bind(&CPU::AND,		this, std::cref(a)),			"ADD A, A",	4, 0 },
		...
		{ 0xae, std::bind(&CPU::XOR,		this, MemRef{hl, mmu}),			"XOR A, (HL)",	8, 0 },
		...
		{ 0xb6, std::bind(&CPU::OR,		this, MemRef{hl, mmu}),			"OR A, (HL)",	8, 0 },
		...
		{ 0xe0, std::bind(&CPU::LD<OffsetRef<0xff00>, BYTE>, this, OffsetRef<0xff00>{n, mmu}, std::cref(a)), "LD (N+0xff00), A", 12, 1 },
		{ 0xe1, std::bind(&CPU::POP,		this, std::ref(hl)),			"POP HL",	12, 0 },
		{ 0xe2, std::bind(&CPU::LD<OffsetRef<0xff00>, BYTE>, this, OffsetRef<0xff00>{c, mmu}, std::cref(a)), "LD (C+0xff00), A", 8, 0 },
		{}, // 0xe3
		{}, // 0xe4
...
DWORD CPU::step() {
	...
	if (debugMode) {
		std::cout << "PC: 0x" << std::setfill('0') << std::setw(4) << std::hex << +(pc-1) << '\n';
